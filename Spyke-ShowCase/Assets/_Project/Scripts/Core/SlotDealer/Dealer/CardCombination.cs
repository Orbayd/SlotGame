...
namespace SpykeGames.Showcase.Core.Dealer
{
    public class CardCombination
    {
        ...
        public static CardCombination GetOrCreate(SlotCombinationType type)
        {
            return SlotCombinationFactory.Create(type);
        }
        private static class SlotCombinationFactory
        {
            private readonly static Dictionary<SlotCombinationType, CardCombination> _slotCombinationMap = new Dictionary<SlotCombinationType,CardCombination>();
            public static CardCombination Create(SlotCombinationType type)
            {
                if(_slotCombinationMap.ContainsKey(type))
                {
                    return _slotCombinationMap[type];
                }
                var slot = new CardCombination();
                switch (type)
                {
